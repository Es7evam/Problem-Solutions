...
int main (void) {
	...
	while(cin >> M.x >> M.y){
		...
		double minim = 1000000000000000.0;
		
		Point prox;
		//cout << "caso:" << endl;
		int mini = 0;
		for(int i=0;i<n;i++){
			if(M.distToLine(v[i], v[(i+1)], prox) < minim){
				minim = M.distToLine(v[i], v[(i+1)], prox);
				mini = i;
			}
		}
		M.distToLine(v[mini], v[(mini+1)], prox);
		
		cout << fixed << setprecision(4) << prox.x << endl << prox.y << endl;
	}
	return 0;
}
