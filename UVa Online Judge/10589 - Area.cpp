...
int main (void) {
	...
	while(cin >> n >> a){
		...
		for(int i=0;i<n;i++){
			cin >> tmp.x >> tmp.y;
			bool flag = true;
			flag = (flag && lowleft.dpp(tmp) < a);
			flag = (flag && lowright.dpp(tmp)< a);
			flag = (flag && topleft.dpp(tmp) < a);
			flag = (flag && topright.dpp(tmp) < a);
			
			if(flag)
				cnt++;
		}
		cout << fixed << setprecision(5) << (double(cnt * a * a)/n) << endl;
